...
from sklearn.metrics import confusion_matrix, roc_curve, auc, roc_auc_score, precision_recall_curve, average_precision_score, multilabel_confusion_matrix
...
def plot_roc_multiclass(y_test, y_prob, n_classes):
...
def plot_pr_multiclass(y_test, y_prob, n_classes):
    ...
    precision["macro"], recall["macro"], _ = precision_recall_curve(y_test.ravel(), y_prob.ravel())
    average_precision["macro"] = average_precision_score(y_test, y_prob, average="macro")
    ...
    lines.append(l)
    labels.append('macro-average Precision-recall (area = {0:0.4f})'
...
def evaluate_model(y_true, y_pred, y_prob, list_classes, n_classes):
    ...
    fpr, tpr, roc_auc = plot_roc_multiclass(y_test, y_prob, n_classes)
    precision, recall, average_precision = plot_pr_multiclass(y_test, y_prob, n_classes)
